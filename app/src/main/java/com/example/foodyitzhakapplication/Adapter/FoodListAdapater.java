...
public class FoodListAdapater extends RecyclerView.Adapter<FoodListAdapater.ViewHolder> {
    ...
    public FoodListAdapater(ArrayList<Foods> items) {
        this.items = items;
    }

    @NonNull
    @Override
    public FoodListAdapater.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    public void onBindViewHolder(@NonNull FoodListAdapater.ViewHolder holder, int position) {
        ...
        holder.timeTxt.setText(String.format("%s min", items.get(position).getTimeValue()));
        holder.priceTxt.setText(String.format("$%s", items.get(position).getPrice()));
        holder.rateTxt.setText(String.format("%s", items.get(position).getStar()));
